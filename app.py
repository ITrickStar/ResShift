...
from scripts.verify import check_dependencies_yaml, check_dependencies_requirements
...
def get_configs(task="realsr", version="v3", scale=4):
    ...
    if task == "realsr":
        ...
        ckpt_path = (
            ckpt_dir / f"resshift_{task}x{scale}_s{_STEP[version]}_{version}.pth"
        )
...
def predict(in_path, task="realsrx4", seed=12345, scale=4, version="v3"):
    ...
    resshift_sampler.inference(
        in_path, out_dir, mask_path=None, bs=1, noise_repeat=False
    )
...
def check_dependencies():
    check_dependencies_yaml("environment.yml")
    check_dependencies_requirements("requirements.txt")
